...
type TaskQuery struct {
	...
	withTask           *CategoryQuery
	withUploadedFile   *UploadedFileQuery
	withCompletedTasks *CompletedTaskQuery
...
func (tq *TaskQuery) QueryTask() *CategoryQuery {
...
// QueryUploadedFile chains the current query on the "uploaded_file" edge.
func (tq *TaskQuery) QueryUploadedFile() *UploadedFileQuery {
	query := (&UploadedFileClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryCompletedTasks chains the current query on the "completed_tasks" edge.
func (tq *TaskQuery) QueryCompletedTasks() *CompletedTaskQuery {
	query := (&CompletedTaskClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withUploadedFile:   tq.withUploadedFile.Clone(),
		withCompletedTasks: tq.withCompletedTasks.Clone(),
...
func (tq *TaskQuery) WithTask(opts ...func(*CategoryQuery)) *TaskQuery {
	query := (&CategoryClient{config: tq.config}).Query()
...
// WithUploadedFile tells the query-builder to eager-load the nodes that are connected to
// the "uploaded_file" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithUploadedFile(opts ...func(*UploadedFileQuery)) *TaskQuery {
	query := (&UploadedFileClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withUploadedFile = query
	return tq
}

// WithCompletedTasks tells the query-builder to eager-load the nodes that are connected to
// the "completed_tasks" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithCompletedTasks(opts ...func(*CompletedTaskQuery)) *TaskQuery {
	query := (&CompletedTaskClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withCompletedTasks = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	var (
		...
		loadedTypes = [3]bool{
			tq.withTask != nil,
			tq.withUploadedFile != nil,
			tq.withCompletedTasks != nil,
		}
	)
	...
	if query := tq.withTask; query != nil {
		if err := tq.loadTask(ctx, query, nodes, nil,
			func(n *Task, e *Category) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withUploadedFile; query != nil {
		if err := tq.loadUploadedFile(ctx, query, nodes, nil,
			func(n *Task, e *UploadedFile) { n.Edges.UploadedFile = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withCompletedTasks; query != nil {
		if err := tq.loadCompletedTasks(ctx, query, nodes,
			func(n *Task) { n.Edges.CompletedTasks = []*CompletedTask{} },
			func(n *Task, e *CompletedTask) { n.Edges.CompletedTasks = append(n.Edges.CompletedTasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadTask(ctx context.Context, query *CategoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *Category)) error {
...
func (tq *TaskQuery) loadUploadedFile(ctx context.Context, query *UploadedFileQuery, nodes []*Task, init func(*Task), assign func(*Task, *UploadedFile)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		fk := nodes[i].UploadedFileID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(uploadedfile.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "uploaded_file_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadCompletedTasks(ctx context.Context, query *CompletedTaskQuery, nodes []*Task, init func(*Task), assign func(*Task, *CompletedTask)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(completedtask.FieldTaskID)
	}
	query.Where(predicate.CompletedTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.CompletedTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withUploadedFile != nil {
			_spec.Node.AddColumnOnce(task.FieldUploadedFileID)
		}
	}
