...
func (uc *UserCreate) AddCompletedLessonIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddCompletedLessonIDs(ids...)
	return uc
}

// AddCompletedLessons adds the "completed_lessons" edges to the CompletedLesson entity.
func (uc *UserCreate) AddCompletedLessons(c ...*CompletedLesson) *UserCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddCompletedLessonIDs(ids...)
}

// AddCompletedTaskIDs adds the "completed_tasks" edge to the CompletedTask entity by IDs.
func (uc *UserCreate) AddCompletedTaskIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddCompletedTaskIDs(ids...)
	return uc
}

// AddCompletedTasks adds the "completed_tasks" edges to the CompletedTask entity.
func (uc *UserCreate) AddCompletedTasks(c ...*CompletedTask) *UserCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddCompletedTaskIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.CompletedLessonsIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.CompletedTasksIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
