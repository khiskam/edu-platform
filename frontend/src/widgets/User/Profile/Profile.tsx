...
export const Profile = () => {
  const { data, isLoading, isRefetching } = UserApi.useGetOneQuery();
  ...
  const { firstName, lastName, role, monthlyActions, statistics } = data.user;

  return (
    <Spin spinning={isRefetching || isPending}>
      <Flex gap={GAP[32]} vertical>
        <User.ProfileData data={{ firstName, lastName, role }} />
        <Divider />
